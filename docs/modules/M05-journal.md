...
* **状态**：文字基础版 ✅ 完成（Simon 2026-09-15 验收）；照片扩展施工中（Simon 2026-09-15 同批指示，expo-image-picker + expo-file-system）
...
| ------------- | --------------- | --- |
| AC-1 \~ AC-10 | 通过（Simon 2026-09-15 会话直接验收，文字基础版；照片扩展另列增量） | Simon |

**Simon 签字**：Simon 日期：2026-09-15


### 2026-09-15 照片收尾增量（本次完善指令授权）
- AC-P1：WHEN 选图超过每篇9张 THE SYSTEM SHALL 明确拒绝，不静默截取或丢失选图。
- AC-P2：WHEN 保存照片 THE SYSTEM SHALL 验证每张实际编码不超过200KB，文件/存储失败保留表单并提示；失败产生的沙盒副本清理。
- AC-P3：WHEN 引用照片缺失 THE SYSTEM SHALL 显示照片不在本机和分享限制，查看页可关闭及前后切换。
网页逐张JPEG缩放压缩，解码失败拒绝整次选图，不吞错误。原生先执行实际尺寸校验；自动压缩待新增expo-image-manipulator技术批准，未批准不添加依赖。缺图展示使用JournalPhoto复用，手账复制/保存副作用仍由页面编排。
