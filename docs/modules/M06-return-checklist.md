...
- AC-2：WHEN 勾选/取消勾选返程条目 THE SYSTEM SHALL 立即持久化（含 sync_queue 记录），进度数字与进度环同步刷新并显示「还差 N 件」。
...
- [ ] **P2（拍板点）删除交互对齐**：行前清单当前是行尾单击 × 直删（无二次确认），行程/账本是 Modal 二次确认。建议本期把行前清单一并对齐为二次确认。请 Simon 拍板。
- [ ] 返程模板种入时机：随旅程创建即种（与行前模板一致，数据一次到位）vs 首次进入返程 Tab 再种（惰性，少写数据）。当前方案：与行前模板一致，创建旅程时同种。
- [ ] 模板四类十条的具体措辞待 Simon 过目（上文列表为草案，CONTEXT.md 演示约定只有五条）。
- [ ] `listChecklistItems` 需增加 phase 参数（默认 'preparation' 保持向后兼容）；该接口尚未正式冻结，仍按契约变更纪律在技术方案段登记。

## ② 技术调研

待需求评审冻结后填写；复用 checklist_item 表与 M02 交互模式，无新选型需求。
