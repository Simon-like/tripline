...
### 2026-09-15 收尾增量
本次完善指令授权统一四旅程规则：导入第五趟开放旅程拒绝并提示，同ID覆盖不新增额度，历史旅程不占额度；此规则替代前文“导入允许超过四趟”的实现选择。native/web仓储导入/创建/编辑同门禁，不变更shared契约。新增两端仓储测试覆盖失败无脏写、重复导入与子ID冲突；原生SQL在host SQLite真实事务运行，不等同设备桥接验收。
